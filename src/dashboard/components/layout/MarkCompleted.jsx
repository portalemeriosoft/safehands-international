...
import { orderStatusPath } from "./../../../api/path";
...
export default function MarkCompleted({
  ...
  const setStatus = () => {
    ...
    if (Number(remaining_payment) !== 0 && paymentType === "") {
      seteErorr("Please select payment type.");
      setLoading(false);
      return;
    } else {
      formData.append("amount", remaining_payment);
      formData.append("remaining_payment_type", paymentType);
    }

    axios
      .post(orderStatusPath, formData)
      .then(({ data }) => {
        markStatusConfirmation[1](false);
  ...
  return (
    <Transition.Root show={markStatusConfirmation[0]} as={Fragment}>
      <Dialog
        as="div"
        className="relative z-10"
        initialFocus={cancelButtonRef}
        onClose={markStatusConfirmation[1]}
      >
        <Transition.Child
          as={Fragment}
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                      ...
                      <div className="mt-2">
                        ...
                        remaining_payment === "0" ? (
                          <p className="text-sm text-gray-500">
                            Are you sure this mission has been completed.
                          </p>
                        ) : (
                          <div className="mt-5">
                            <p className="text-sm text-gray-500 mb-2">
                              <b>Remaining Amount:</b> £{remaining_payment}
                            </p>
                            <label
                              htmlFor="remaining_payment_type"
                              className="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                            >
                              How did you received the remaining payment?
                            </label>
                            <select
                              onChange={(e) => handleChange(e)}
                              id="remaining_payment_type"
                ...
                <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  ...
                  <button
                    ...
                    onClick={() => markStatusConfirmation[1](false)}
                    ref={cancelButtonRef}
                  >
                    Leave
                  </button>
