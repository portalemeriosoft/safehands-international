	...
	function checkBookingForm($bookingFormId,$currency=null,$displayError=false)
	{
		...
		(
			...
			(
				...
				(				
					'number'=>__('1','chauffeur-booking-system'),
					'label'=>__('VEHICLE','chauffeur-booking-system')
				),
				'button'=>array
				(
			...
			(
				...
				(
					...
					'label'=>__('DETAILS','chauffeur-booking-system')
				),
				'button'=>array
				(
			...
			(
				...
				(
					...
					'label'=>__('PAYMENT','chauffeur-booking-system')
				),
				'button'=>array
				(
	...
	function goToStep()
	{
		...
		if(!isset($response['error']))
		{
			if($data['step_request']>3)
			{
				...
				if (empty($_POST['checkbox_req'])) {
    $this->setErrorLocal(
        $response,
        CHBSHelper::getFormName('checkbox_req', false),
        __('Required: Please accept the terms & conditions to continue with your booking.', 'chauffeur-booking-system')
    );

    
    $this->data['error_field_class']['checkbox_req'] = 'chbs-error';

   
    $this->data['error_messages']['checkbox_req'] = __('Required: Please accept the terms & conditions to continue with your booking.', 'chauffeur-booking-system');
}
	...
	function createSummaryPriceElement($data,$bookingForm,&$price)
	{
					...
					'
						...
						<div class="chbs-clear-fix chbs-coupon-code-section">
							<div class="chbs-form-field">
								...
								<input maxlength="12" name="'.CHBSHelper::getFormName('coupon_code',false).'" placeholder="Enter coupon code" value="'.esc_html(array_key_exists('coupon_code',$data) ? $data['coupon_code'] : '').'" type="text">
							 <div class="coupon-error" style=" color: red; font-size: 12px;"></div>
  
							</div>
								<a href="#" class="chbs-button chbs-button-style-2 chbs-button-coupon" id="applyButton" >
								'.__('Apply','chauffeur-booking-system').'
		...
		'
			<div class="chbs-summary-price-element-total">
				...
				<div class="price-note-bottom">Do you need help? <img src="https://stagingmockup.agency/drivero-dev/wp-content/uploads/2017/11/chaticon.png" width="15"><span onclick="Tawk_API.toggle();" style="cursor: pointer;"
>Start a Chat!</span> </div>
<div class="coupen-move">'.$couponHtml.'</div>
		';			
	...
	function createSummary($data,$bookingForm)
	{
		...
		switch($data['step_request'])
		{
			...
			case 4:
				...
				$response[2]=$thumbnailHtml.$BookingFormSummary->create(__('Vehicle info','chauffeur-booking-system'),2).$gratuityHtml.$priceHtml;
	...
	function createVehicle($data,&$priceToSort,&$urlAddress)
	{
		...
		if((int)$data['booking_form']['meta']['passenger_number_vehicle_list_enable']===1)
		{
			...
			'
				...
				<p class="inner-selecet-process1 inner-selecet-process">	<span class="chbs-meta-icon-people"></span>Up to <span class="chbs-circle">'.$data['vehicle']['meta']['passenger_count'].'</span>	Passangers</p>			
			';
		}
		if((int)$data['booking_form']['meta']['suitcase_number_vehicle_list_enable']===1)
		{
			...
			'
				...
				<p class="inner-selecet-process2 inner-selecet-process"><span class="chbs-meta-icon-bag"></span><span class="chbs-circle">'.$data['vehicle']['meta']['bag_count'].'</span> medium suitcases</p>			
			';
		}
		if($Validation->isNotEmpty($htmlVehicleInfo))
		{
			$htmlVehicleInfo=
			'
				<div class="chbs-vehicle-content-meta-info inner-selecet-process-passanger-container">
					<div>
						'.$htmlVehicleInfo.'
					</div>
				</div>				
		...
		$html=
		'
			<div class="chbs-vehicle chbs-clear-fix vehicle-list-wrapper" data-id="'.esc_attr($data['vehicle_id']).'" data-base_location_cooridnate_lat="'.esc_attr($data['vehicle']['meta']['base_location_coordinate_lat']).'"  data-base_location_cooridnate_lng="'.esc_attr($data['vehicle']['meta']['base_location_coordinate_lng']).'">
				...
				<div class="chbs-vehicle-content">
					...
					<p class="text-wl2">✓ Includes VAT, fees & tip</p>
						'.$htmlSelect.'
						</div>
	...
	function createClientFormSignUp($bookingForm,$userData=array())
	{
		...
		'
			<div'.CHBSHelper::createCSSClassAttribute($class).'>
				...
				<div class="chbs-box-shadow form-step3-process-form">
				
					<div class="chbs-clear-fix">
						<label class="chbs-form-label-group">'.esc_html__('Contact details','chauffeur-booking-system').'</label>
						<div class="chbs-clear-fix gaping-between-fileds">
						<div class="chbs-form-field chbs-form-field-width-50">
							<label>'.esc_html__('First name *','chauffeur-booking-system').'</label>
							<input placeholder="Enter your first name" maxlength="20" type="text" name="'.CHBSHelper::getFormName('client_contact_detail_first_name',false).'" value="'.esc_attr($data['client_contact_detail_first_name']).'"/>
						</div>
						<div class="chbs-form-field chbs-form-field-width-50">
							<label>'.esc_html__('Last name *','chauffeur-booking-system').'</label>
							<input placeholder="Enter your last name" maxlength="20" type="text" name="'.CHBSHelper::getFormName('client_contact_detail_last_name',false).'" value="'.esc_attr($data['client_contact_detail_last_name']).'"/>
						</div>
						</div>
					</div>
					...
					<div class="chbs-clear-fix gaping-between-fileds">
						<div class="chbs-form-field chbs-form-field-width-50">
							...
							<input placeholder="Enter your email" maxlength="45" type="text" name="'.CHBSHelper::getFormName('client_contact_detail_email_address',false).'"  value="'.esc_attr($data['client_contact_detail_email_address']).'"/>
						</div>
						<div class="chbs-form-field chbs-form-field-width-50">
							...
							<input type="tel" name="'.CHBSHelper::getFormName('client_contact_detail_phone_number_placeholder',false).'"  value="'.esc_attr($data['client_contact_detail_phone_number_placeholder']).'"/>
					...
					<div class="chbs-clear-fix text-area-form-filed">
						<div class="chbs-form-field">
							<label>'.esc_html__('Notes','chauffeur-booking-system').'</label>
							<textarea placeholder="Providing additional information will enable us to offer you the best possible service. Please do not enter any confidential information.





" name="'.CHBSHelper::getFormName('comment',false).'">'.esc_html(array_key_exists('comment',$data) ? $data['comment'] : CHBSRequestData::post('chbs_comment_hidden')).'</textarea>
						</div>
					</div>
		...
		$html.=
		'
					<div class="chbs-clear-fix billing-box-container">
						<label class="chbs-form-label-group">
							'.$checkboxHtml.'
	...
	function createPaymentForm($bookingForm)
	{
		...
		if(((array_key_exists('payment_woocommerce',$bookingForm['dictionary'])) && (count($bookingForm['dictionary']['payment_woocommerce']))) || ((array_key_exists('payment',$bookingForm['dictionary'])) && (count($bookingForm['dictionary']['payment']))))
		{
			if((int)$bookingForm['meta']['price_hide']===0)
			{
				...
				';
					$BookingFormElement=new CHBSBookingFormElement();
					echo $BookingFormElement->createAgreement($bookingForm['meta'],$data['service_type_id']);
    ...
    <div class="custom-check-box-wrapper"><label class="chbs-form-label-group custom-check-box" for="checkbox_req">
        <input type="checkbox" id="checkbox_req" name="checkbox_req" value="1"
            class="' . (isset($this->data['error_field_class']['checkbox_req']) ? $this->data['error_field_class']['checkbox_req'] : '') . '"> 
        I accept the & <a href="terms-conditions/" target="_blank" class="terms-link"><strong>Terms & Conditions - Booking Conditions</strong></a>. *
    </label>
    
    <!-- Display error message below the checkbox if it exists -->
    ' . (isset($this->data['error_messages']['checkbox_req']) ? '<p class="chbs-error-message">' . $this->data['error_messages']['checkbox_req'] . '</p>' : '') . ' </div>';
